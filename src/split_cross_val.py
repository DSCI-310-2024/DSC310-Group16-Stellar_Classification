...
def split_cross_val(preprocessed_data, target, split=0.75, folds=5):
    ...
    results["logistic"] = pd.Series({col: f"{mean:.3f}" for col, mean in zip(lr_df.index, lr_df)})
    ...
    results["random_forest"] = pd.Series({col: f"{mean:.3f}" for col, mean in zip(rfc_df.index, rfc_df)})
