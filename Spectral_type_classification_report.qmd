...
class type. Our model can only classify stars into 4 main classes due to
the small sample size. It is recommended that further study using larger
sample sizes and methods to improve the classification model.
...
the Morgan–Keenan (MK) system (Morgan et al. 1943) which group stars
into 7 classes based on their spectral characteristics. Under the MK
system, astronomers analyse electromagnetic radiation from stars to
determine its class. These electromagnetic spectrum have dark lines to
determine which and how abundant elements are present in the star. The 7
...
*Sloan* system (add reference) used by the Sloan Digital Sky Survey. The
system measures the intensity of electromagnetic radition from stars at
5 bands: - *u* (345nm) - *g* (475nm which is a light blue in the visible
spectrum) - *r* (622nm which is orange) - *i* (763nm which is deep
red) - *z* (905nm)
...
following Python packages were used to perform the analysis: matplotlib
(Hunter, 2007), scikit-learn (Pedregosa et al., 2011) and Pandas
(McKinney, 2010).
...
sklearn and pandas for our classification analysis as well as matplotlib
...
only_stars_data = exoplanet_data.dropna(subset = ['st_spectype',
                                                  ...
                                                  'sy_imag',
                                                  'sy_zmag'])
only_stars_data.info #ok so i did not include this in the makefile
```
...
![Histogram of the Value Counts of Our Star
Types](../results/star_count_hist.png){#fig-star-count width="50%"}

```{python}
#| label: tbl-sy-umag
#| tbl-cap: Table of sy_umag Features

sy_umag = pd.read_csv("../results/sy_umag.csv!!!!!!!")
Markdown(sy_umag.to_markdown(index = False))
```

![Box Plot of `sy_umag`](../results/sy_umag.png){#fig-sy_umag
width="50%"}

For *M*-class of stars, the magnitude of the *u*-band is much higher
than the remaining classes at 17.3 at the median.

```{python}
#| label: tbl-sy-gmag
#| tbl-cap: Table of sy_gmag Features

sy_gmag = pd.read_csv("../results/sy_gmag.csv")
Markdown(sy_gmag.to_markdown(index = False))
```

![Box Plot of `sy_gmag`](../results/sy_gmag.png){#fig-sy_gmag
width="50%"}

Again, for *M*-class of stars, the magnitude of the *g*-band is much
higher than the remaining classes at 15.3 at the median.

```{python}
#| label: tbl-sy-rmag
#| tbl-cap: Table of sy_rmag Features

sy_rmag = pd.read_csv("../results/sy_rmag.csv")
Markdown(sy_rmag.to_markdown(index = False))
```

![Box Plot of `sy_rmag`](../results/sy_rmag.png){#fig-sy_rmag
width="50%"}

Again, for *M*-class of stars, the magnitude of the *r*-band is higher
than the remaining classes at 13.4 at the median.

```{python}
#| label: tbl-sy-imag
#| tbl-cap: Table of sy_imag Features

sy_imag = pd.read_csv("../results/sy_imag.csv")
Markdown(sy_imag.to_markdown(index = False))
```

![Box Plot of `sy_imag`](../results/sy_imag.png){#fig-sy_imag
width="50%"}
...
sy_zmag = pd.read_csv("../results/sy_zmag.csv")
Markdown(sy_zmag.to_markdown(index = False))
```

![Box Plot of `sy_zmag`](../results/sy_zmag.png){#fig-sy_zmag
width="50%"}
...
lr_cvs = pd.read_csv("../results/logistic_regression_df.csv!!!!!!!")
...
How well the classifier is identifying each class. - Which classes are
most frequently confused with each other. - Overall accuracy, along with
...
lr_cm = pd.read_csv("../results/confusion_matrix.csv!!!!!!!")
Markdown(lr_cm.to_markdown(index = False))
```

![Visualization of Our Confusion
Matrix](../results/confusion_matrix.png){#fig-cm width="50%"} #!!!!!!

I do not have the table for cross_val_scores on here!!!!!!

```{python}
#| label: tbl-accuracy score
#| tbl-cap: Table of Accuracy Score From Random Forest Classifier

rfc_accuracy = pd.read_csv("../results/confusion_matrix.csv!!!!!!!")
...
rfc_cvs = pd.read_csv("../results/logistic_regression_df.csv!!!!!!!")
...
into 4 classes due to the limited sample size. However these 4 classes
make up about 99.8% of stellar population (Ledrew, 2001) so being unable
to classify stars into remaining 3 classes isn't as big of an issue.
Looking at the confusion matrix, we can see that our model tend to
classify stars as cooler than they actually are (e.g: 9 stars were
classified as *G* but were actually *F* class). In order to improve this
model, a larger sample size would help like using the Sloan Digital Sky
Survey dataset instead. Another way to improve the model is to explore
other classification methods such as k nearest neighbours. Finally,
using another photometric system such as UBV could help since the bands
are more seperated resulting in larger difference in magnitudes between
star classes. More research into other classification methods could most
likely yield higher accuracy.

## References

-   Morgan, W. W., Keenan, P. C., & Kellman, E. (1942). An atlas of
    stellar spectra. University of Chicago.
-   Duan, F. Q., Liu, R., Guo, P., Zhou, M. Q., & Wu, F. C. (2009).
    Automated spectral classification using template matching. Research
    in Astronomy and Astrophysics, 9(3), 341.
-   Van Rossum, G., & Drake Jr, F. L. (1995). Python tutorial (Vol.
    620). Amsterdam, The Netherlands: Centrum voor Wiskunde en
    Informatica.
...
-   Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion, B.,
    ...
    2825-2830.
-   McKinney, W. (2010, June). Data structures for statistical computing
    in python. In Proceedings of the 9th Python in Science Conference
    (Vol. 445, No. 1, pp. 51-56).
-   Ledrew, G. (2001). The real starry sky. Journal of the Royal
    Astronomical Society of Canada, Vol. 95, p. 32, 95, 32.
